...
gene_expression = pd.read_csv(
    BASE_PATH / "data" / "gene-expression" / "kinase_RNA_seq.csv", index_col=0
).drop("GrowthPhase", axis=1)
...
normalized_expression = pd.read_csv(
    BASE_PATH / "data" / "quant_normed" / "xprs_norm.tsv", sep="\t", index_col=0
).transpose()
...
for condition in expr_dict:
    expr_dict[condition] = gene_expression.loc[
        gene_expression.index.str.startswith(condition)
    ]
    norm_expr_dict[condition] = normalized_expression.loc[
        normalized_expression.index.str.startswith(condition)
    ]
...
iek1011 = metworkpy.read_model(
    BASE_PATH / "data" / "Models" / "iEK1011_m7H10_media.json"
)
...
wt_expression = (
    gene_expression.loc[gene_expression.index.str.startswith("WT")].median(axis=0)
    + 1e-10
)
...
for condition in rxn_weight_dict:
    ...
    if p.exists():
        ...
        gene_weights = metworkpy.utils.expr_to_weights(
            expression=expr_dict[condition],
            quantile=0.15,
            subset=iek1011.genes.list_attr("id"),
            sample_axis=0,
        )
        rxn_weight_dict[condition] = metworkpy.parse.gene_to_rxn_weights(
            iek1011, gene_weights
        )
        rxn_weight_dict[condition].to_json(p)
    if norm_p.exists():
        norm_rxn_weight_dict[condition] = pd.read_json(norm_p, typ="series")
    else:
        norm_gene_weights = metworkpy.utils.expr_to_weights(
            expression=norm_expr_dict[condition],
            quantile=0.15,
            subset=iek1011.genes.list_attr("id"),
            sample_axis=0,
        )
        norm_rxn_weight_dict[condition] = metworkpy.parse.gene_to_rxn_weights(
            iek1011, norm_gene_weights
        )
        norm_rxn_weight_dict[condition].to_json(norm_p)
    if condition != "WT":
        diff_p = (
            BASE_PATH / "cache" / "diff_rxn_weights" / f"{condition}_rxn_weights.json"
        )
        if diff_p.exists():
            diff_rxn_weight_dict[condition] = pd.read_jason(diff_p, typ="series")
        else:
            log2fc = np.log2(
                gene_expression.loc[gene_expression.index.str.startswith(condition)]
                .median(axis=0)
                .div(wt_expression)
                + 1e-10
            )
            diff_gene_weights = log2fc.apply(
                lambda x: -1 if x <= -1 else (1 if x >= 1 else 0)
            )
            diff_rxn_weight_dict[condition] = metworkpy.parse.gene_to_rxn_weights(
                iek1011, diff_gene_weights
            )
...
biomass_growth_df = pd.DataFrame(
    np.NaN,
    index=rxn_weight_dict.keys(),
    columns=["simple_bounds", "subset_model", "fva_model", "milp_model"],
    dtype="float",
)
norm_biomass_growth_df = pd.DataFrame(
    np.NaN,
    index=norm_rxn_weight_dict.keys(),
    columns=["simple_bounds", "subset_model", "fva_model", "milp_model"],
    dtype="float",
)
diff_biomass_growth_df = pd.DataFrame(
    np.NaN,
    index=norm_rxn_weight_dict.keys(),
    columns=["simple_bounds", "subset_model", "fva_model", "milp_model"],
    dtype="float",
...
def imat_per_method(
    method, weights_dict, results_path, biomass_growth_df, skip_list=None
):
    ...
    for condition in weights_dict:
        ...
        if p.exists():
            ...
            try:
                model = metworkpy.imat.generate_model(
                    model=iek1011.copy(),
...
biomass_growth_df[~(biomass_growth_df.index == "WT")].div(
    biomass_growth_df.loc["WT"], axis=1
).to_csv(BASE_PATH / "results" / "expr_models" / "biomass_growth_prop.csv")
# Qnorm results
norm_biomass_growth_df.to_csv(
    BASE_PATH / "results" / "norm_expr_models" / "norm_biomass_growth.csv"
)
norm_biomass_growth_df[~(norm_biomass_growth_df.index == "WT")].div(
    norm_biomass_growth_df.loc["WT"], axis=1
).to_csv(BASE_PATH / "results" / "norm_expr_models" / "norm_biomass_growth_prop.csv")
# Differential Expression Results
diff_biomass_growth_df.to_csv(
    BASE_PATH / "results" / "diff_expr_models" / "diff_biomass_growth.csv"
)
diff_biomass_growth_df[~(diff_biomass_growth_df.index == "WT")].div(
    diff_biomass_growth_df.loc["WT"], axis=1
).to_csv(BASE_PATH / "results" / "diff_expr_models" / "diff_biomass_growth_prop.csv")
...
def essentiality_analysis(
    model_dir: pathlib.Path, cond_list: list[str], wt: str, out_dir: pathlib.Path
) -> None:
    wt_model = metworkpy.read_model(str(model_dir / f"iek1011_{wt}_model.json"))
    flux_res_df = pd.DataFrame(
        np.NaN,
        dtype="float",
        index=wt_model.genes.list_attr("id"),
        columns=[wt] + cond_list,
    )
    prop_res_df = pd.DataFrame(
        np.NaN,
        dtype="float",
        index=wt_model.genes.list_attr("id"),
        columns=[wt] + cond_list,
    )
    ess_res_df = pd.DataFrame(
        np.NaN,
        dtype="float",
        index=wt_model.genes.list_attr("id"),
        columns=[wt] + cond_list,
    ...
    for cond in [wt] + cond_list:
        ...
        try:
            ...
            prop_res_df.loc[flux_res_df.index, cond] = (
                flux_res_df.loc[:, cond] / max_obj
            )
...
for method in ["simple_bounds", "subset_model", "fva_model", "milp_model"]:
    essentiality_analysis(
        ...
        / "results"
        / "essentiality"
        / f"{EXPRESSION_METHOD}_essentiality"
        / method,
    )
